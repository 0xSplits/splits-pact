...
#### DD-7 — REJECTED (2026-07-19, same day): launcher admin share

A launcher provision (1 unit locked at mint + non-fee Clanker admin roles
to the launcher's wallet) was decided and reverted within the day: **KISS
for v1 — no edge cases.** No special units for anyone; all 1000 units
quantize across backers by weight; every Clanker admin role lands on the
immutable settlement contract (or is renounced) so the "creator provably
can't keep or change anything" claim stays absolute. Revisit post-v1 only
if real launches surface a concrete need (e.g. metadata updates).
